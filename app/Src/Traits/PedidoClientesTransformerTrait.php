...
trait PedidoClientesTransformerTrait 
{
        ...
        return collect($pc->items)->map(function($i){
            ...
            if ($i->product()->exists()) {
                ...
                return [
                    ...
                    'unit_price_invoiceA' => ($i->neto_import) / $quantity,
                    'unit_price_invoiceB' => ($i->neto_import + $i->iva_import) / $quantity,
                    ...
                    'total_Presupuesto' => ($i->neto_import),
    ...
    public function total($pc)
    {
        ...
        {
            return $this->DisplayToUserCurrency($pc->items->sum('neto_import'));
        }

        $total = $pc->items->sum('neto_import') + $pc->items->sum('iva_import');
    ...
    public function invoices($pc)
    {
        ...
        $invoices = $pc->invoices->map(function ($invoice){
            ...
            return [
                ...
                'import' => ($invoice->voucher->name === 'PRESUPUESTO VENTA') ? $invoice->items->sum('neto_import') : $invoice->items->sum('total')
            ];
        });
