...
class PdfSaleInvoiceTransformer extends TransformerAbstract
{
    ...
    private function detail_of_totals($si)
    {
        ...
        if ((int)$si->voucher_id == 88 ) {
            $total = $totals_collection->sum('neto_import') + $payment_type_aditional + $si->percerp_iibb;
        } else{
            $total = $totals_collection->sum('neto_import') + $totals_collection->sum('iva_import') + $payment_type_aditional + $si->percerp_iibb;
        }
