...
class SaleInvoiceController extends Controller
{
    ...
    public function search($request)
    {
        ...
        $invoices = $invoices->whereBetween('cbte_fch', [$this->ShortDateToYyyymmddFormat($request->from), $this->ShortDateToYyyymmddFormat($request->to)]);
    ...
    public function save_presupuesto()
    {
        ...
        collect($presupuesto['items'])->map(function($item) use ($si) {
            ...
            product_history($product, 'descuenta stock', auth()->user()->id, $array);
        });
