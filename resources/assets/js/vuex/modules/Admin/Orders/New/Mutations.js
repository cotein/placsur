...
export const NEW_ORDER_SET_DISCOUNT_PRODUCT = (state, payload) => {
    state.order.products[payload.index].discount = parseFloat(payload.value);
}
...
export const NEW_ORDER_SET_PRICE_PRODUCT = (state, payload) => {
    state.order.products[payload.index].unit_price = payload.value;
    let product = state.order.products[payload.index];
    if (product.isCHP) {
        product.neto = (parseFloat(product.unit_price) * parseFloat(product.mts_to_invoiced)) - parseFloat(product.discount) ;    
    }else{
        product.neto = (parseFloat(product.unit_price) * parseFloat(product.quantity)) - parseFloat(product.discount) ;
    }
...
export const NEW_ORDER_SET_TOTALS_FROM_PRODUCT = (state, index) => {
    ...
    if (product != undefined) {
        ...
        if (state.order.products[index].isCHP) {
            state.order.products[index].neto = (parseFloat(state.order.products[index].unit_price) * parseFloat(state.order.products[index].mts_to_invoiced)) - parseFloat(state.order.products[index].discount) ;    
        }else{
            state.order.products[index].neto = (parseFloat(state.order.products[index].unit_price) * parseFloat(state.order.products[index].quantity)) - parseFloat(state.order.products[index].discount) ;
        }
        
        state.order.products[index].iva_import = state.order.products[index].neto * parseFloat(state.order.products[index].iva.percentage) / 100;
        
        const total = parseFloat(state.order.products[index].neto) + parseFloat(state.order.products[index].iva_import);
...
export const NEW_ORDER_SET_PAYMENT_TYPE = (state, value) => {
    state.order.payment.id = value.id;
    state.order.payment.name = value.name;
    state.order.payment.percentage = value.percentage;
}
...
export const NEW_ORDER_SET_SHIPPING_VALUE = (state, value) => {
    ...
    console.log("🚀 ~ file: Mutations.js ~ line 195 ~ state.order.shipping.value ", state.order.shipping.value )
}
