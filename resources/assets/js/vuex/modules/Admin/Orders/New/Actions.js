...
export const newOrdersetUnitPriceProduct = ({commit}, payload) => {
    commit('NEW_ORDER_SET_UNIT_PRICE_PRODUCT', payload);
    commit('NEW_ORDER_SET_TOTALS_FROM_PRODUCT', payload.index);
}

export const newOrderSetQuantityProduct = ({commit}, payload) => {
...
export const calculeNetoAction = ({state}) => {
    ...
    const total = products.reduce( (acc, item) => {
        ...
        if (item.isCHP) {
            return acc + (parseFloat(item.unit_price) * parseFloat(item.mts_to_invoiced)) - parseFloat(item.discount) ;    
        }else{

            return acc + parseFloat(item.unit_price) * (parseFloat(item.quantity) + parseFloat(item.rounded_mts));
        }
    }); 
...
export const calculeTotalAction = ({state}) => {
    ...
    let total_order = total + shipping + payment;
...
export const restore_pedido = async (_, order_id) => {
    ...
    try {
        ...
        let response = await axios.put('/api/pedido_cliente/restore_pedido', {
            order_id : order_id
        });
